...
public class WebSocketDeflateSpec extends WebSocketBehaviors {
  final WsSettings wsSettings = WsSettings.defaultCompression();
  final Uri wsUri = Uri.parse("ws://127.0.0.1:33556/");

  @Override
  protected ServiceRef bind(HttpEndpoint endpoint, HttpService service) {
    return endpoint.bindHttp("127.0.0.1", 33554, service, this.wsSettings.httpSettings());
  }

  @Override
  protected SocketRef connect(HttpEndpoint endpoint, final WebSocket<?, ?> socket) {
