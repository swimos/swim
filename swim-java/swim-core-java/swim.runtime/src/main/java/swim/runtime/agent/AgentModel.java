...
public class AgentModel extends AgentNode {
  ...
  public AgentModel(AgentType<?> agentType, Value props) {
  ...
  public void setNodeContext(NodeContext nodeContext) {
    ...
    addAgentView(materializeAgent(this.agentType, this.props));
  }
  ...
  public <T> T unwrapNode(Class<T> nodeClass) {
    if (nodeClass.isAssignableFrom(getClass())) {
      ...
      if (views instanceof AgentView) {
        return ((AgentView) views).unwrapNode(nodeClass);
      } else if (views instanceof AgentView[]) {
        final AgentView[] viewArray = (AgentView[]) views;
        for (int i = 0, n = viewArray.length; i < n; i += 1) {
          final T node = viewArray[i].unwrapNode(nodeClass);
          if (node != null) {
            return node;
          }
        }
  ...
  public Value agentKey() {
    final String agentName = this.agentType.name();
    if (agentName != null) {
      return Text.from(agentName);
    } else {
      return Value.absent();
    }
  }
  ...
  public AgentView materializeAgent(AgentFactory<?> agentFactory, Value props) {
    final AgentView view = new AgentView(this, props);
    SwimContext.setAgentContext(view);
    final Agent agent = agentFactory.createAgent(view);
    SwimContext.clear();
    view.setAgent(agent);
    return view;
  }
    ...
    do {
      ...
      if (oldViews instanceof AgentView) {
        ...
        if (props.equals(oldView.props())) {
          ...
          if (view == null) {
            view = materializeAgent(agentFactory, props);
          }
          newViews = new AgentView[]{oldView, view};
        }
      } else if (oldViews instanceof AgentView[]) {
