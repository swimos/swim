...
public abstract class MqttSocketBehaviors {
  protected abstract ServiceRef bind(MqttEndpoint endpoint, MqttService service);

  protected abstract SocketRef connect(MqttEndpoint endpoint, MqttSocket<?, ?> socket);
  ...
  protected void benchmark(int connections, final long duration, final String payload) {
    ...
    try {
      ...
      for (int connection = 0; connection < connections; connection += 1) {
        connect(endpoint, new AbstractMqttSocket<String, String>() {
          @Override
          public void didConnect() {
            read(Utf8.stringParser());
          }
