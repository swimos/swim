...
public class SocketSettingsSpec {
  void assertDecodes(Value actualValue, SocketSettings expected) {
    final SocketSettings actual = SocketSettings.form().cast(actualValue);
    assertEquals(actual, expected);
  }

  @Test
  public void decodesStandardSettings() {
    assertDecodes(Record.empty(), SocketSettings.standard());
  }

  @Test
  public void decodesStandardTcpSettings() {
    assertDecodes(Record.of(Record.of(Attr.of("tcp"))), SocketSettings.standard());
  }
  ...
  public void decodesTcpSettings() {
    assertDecodes(Record.of(Record.of(Attr.of("tcp"),
                  ...
                  SocketSettings.from(new TcpSettings(true, true, 2, 3, 5, 7)));
  }

  @Test
  public void decodesStandardTlsSettings() {
    final SocketSettings settings = SocketSettings.form().cast(Record.of(Record.of(Attr.of("tls"))));
    assertEquals(settings.tcpSettings(), TcpSettings.standard());
  }

  @Test
  public void decodesTlsAndTcpSettings() {
    final SocketSettings settings = SocketSettings.form().cast(
