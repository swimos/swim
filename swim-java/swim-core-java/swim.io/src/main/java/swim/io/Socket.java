 ...
 *
 * A {@code Socket} interfaces with the underlying asynchronous networking
 * system via a {@link SocketContext}.  The socket context invokes I/O
 * callbacks on the {@code Socket} when the underlying network socket is ready
 * to perform I/O operations permitted by the socket context's {@link
 * FlowControl}.
 */
public interface Socket {
  /**
   * Returns the network socket context to which this {@code Socket} is bound;
   * returns {@code null} if this {@code Socket} is unbound.
   */
  SocketContext socketContext();

  /**
   * Sets the network socket context to which this {@code Socket} is bound.
   */
  void setSocketContext(SocketContext context);

  /**
   * Returns the number of idle milliseconds after which this {@code Socket}
  ...
  /**
   * I/O callback invoked by the socket context asking this {@code Socket} to
   * read input data out of the socket context's {@link
   * SocketContext#inputBuffer() inputBuffer}.  May be invoked concurrently to
   * other I/O callbacks, but never concurrently with other {@code doRead} calls.
   */
  void doRead();

  /**
   * I/O callback invoked by the socket context asking this {@code Socket} to
   * write output data into the socket context's {@link
   * SocketContext#outputBuffer() outputBuffer}.  May be invoked concurrently
   ...
   * SocketContext#become(Socket) become} a new {@code socket} implementation.
   */
