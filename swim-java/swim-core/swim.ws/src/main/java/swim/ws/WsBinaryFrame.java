...
public final class WsBinaryFrame<T> extends WsDataFrame<T> implements WriteSource {
  ...
  WsBinaryFrame(@Nullable T payload, Codec<T> codec) {
  ...
  public boolean equals(Object other) {
    if (this == other) {
      return true;
    } else if (other instanceof WsBinaryFrame<?> that) {
  ...
  public static <T> WsBinaryFrame<T> of(@Nullable T payload, Codec<T> codec) {
    return new WsBinaryFrame<T>(payload, codec);
  }

  public static WsBinaryFrame<byte[]> of(byte @Nullable [] payload) {
    return new WsBinaryFrame<byte[]>(payload, Binary.byteArrayCodec());
  }
  ...
  public static WsBinaryFrame<ByteBuffer> of(@Nullable ByteBuffer payload) {
    ...
    return new WsBinaryFrame<ByteBuffer>(payload, Binary.byteBufferCodec());
  }
