...
public class WsEncoderTests {
  ...
  public void encodeUnmaskedTinyFrameToTinyBuffer() {
    ...
    WsTestEncoder.server().encodeMessage(output, WsTextFrame.of("Hello")).assertDone();
    assertEquals(Base16.parseByteBuffer("810548656C6C6F").getNonNullUnchecked(), output.get());
  }
  ...
  public void encodeMaskedTinyFrameToTinyBuffer() {
    ...
    WsTestEncoder.client(0x37FA213D).encodeMessage(output, WsTextFrame.of("Hello")).assertDone();
    assertEquals(Base16.parseByteBuffer("818537FA213D7F9F4D5158").getNonNullUnchecked(), output.get());
  }
  ...
  public void encodeLongestTinyUnmaskedFrame() {
    ...
    WsTestEncoder.server().encodeMessage(output, WsBinaryFrame.of(ByteBuffer.allocate(payloadSize))).assertDone();
  ...
  public void encodeLongestTinyMaskedFrame() {
    ...
    final BinaryOutputBuffer output = new BinaryOutputBuffer(ByteBuffer.allocate(headerSize + payloadSize));
    WsTestEncoder.client(maskingKey).encodeMessage(output, WsBinaryFrame.of(ByteBuffer.allocate(payloadSize))).assertDone();
    assertEquals(frame, output.get());
  }
