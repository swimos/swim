...
@Public
@Since("5.0")
public final class HttpStatus implements ToSource, ToString {
  ...
  public static HttpStatus create(int code) {
    switch (code) {
  ...
  public static Parse<HttpStatus> parse(Input input) {
    return ParseHttpStatus.parse(input, 0, null, 1);
  }

  public static Parse<HttpStatus> parse() {
    return new ParseHttpStatus(0, null, 1);
  }
...
final class ParseHttpStatus extends Parse<HttpStatus> {
  ...
  ParseHttpStatus(int code, @Nullable StringBuilder phraseBuilder, int step) {
    this.code = code;
  ...
  public Parse<HttpStatus> consume(Input input) {
    return ParseHttpStatus.parse(input, this.code, this.phraseBuilder, this.step);
  }
  ...
  static Parse<HttpStatus> parse(Input input, int code,
    ...
    if (step == 5) {
      phraseBuilder = Assume.nonNull(phraseBuilder);
      while (input.isCont()) {
        c = input.head();
      ...
      if (input.isReady()) {
        return Parse.done(HttpStatus.create(code, phraseBuilder.toString()));
      }
    }
    if (input.isError()) {
      return Parse.error(input.getError());
    }
    return new ParseHttpStatus(code, phraseBuilder, step);
  }
