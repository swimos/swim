...
@Internal
public final class WriteChildExpr extends Write<Object> {
  ...
  public WriteChildExpr(ExprWriter writer, TermForm<?> form, Term scope, Term key,
  ...
  public Write<Object> produce(Output<?> output) {
    return WriteChildExpr.write(output, this.writer, this.form, this.scope,
                                this.key, this.precedence, this.write,
                                this.step);
  }

  public static Write<Object> write(Output<?> output, ExprWriter writer,
                                    TermForm<?> form, Term scope, Term key,
    ...
    if (step == 5) {
      if (write == null) {
        write = writer.writeTerm(output, form, key);
      } else {
        write = write.produce(output);
      }
      if (write.isDone()) {
    ...
    return new WriteChildExpr(writer, form, scope, key, precedence, write, step);
  }
