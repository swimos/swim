...
@Internal
public final class ParseBitwiseAndExpr extends Parse<Term> {
  ...
  public ParseBitwiseAndExpr(ExprParser parser, TermForm<?> form,
  ...
  public Parse<Term> consume(Input input) {
    return ParseBitwiseAndExpr.parse(input, this.parser, this.form,
  ...
  public static Parse<Term> parse(Input input, ExprParser parser, TermForm<?> form,
    ...
    if (step == 1) {
      if (parseLhs == null) {
        parseLhs = parser.parseComparisonExpr(input, form);
      } else {
    ...
    do {
      ...
      if (step == 3) {
        if (parseRhs == null) {
          parseRhs = parser.parseComparisonExpr(input, form);
        } else {
          parseRhs = parseRhs.consume(input);
        }
        if (parseRhs.isDone()) {
          parseLhs = Parse.done(new BitwiseAndExpr(Assume.nonNull(parseLhs).getNonNullUnchecked(),
    ...
    return new ParseBitwiseAndExpr(parser, form, parseLhs, parseRhs, step);
  }
