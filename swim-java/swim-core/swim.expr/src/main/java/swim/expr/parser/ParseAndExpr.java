...
@Internal
public final class ParseAndExpr extends Parse<Term> {
  ...
  public ParseAndExpr(ExprParser parser, TermForm<?> form,
  ...
  public Parse<Term> consume(Input input) {
    return ParseAndExpr.parse(input, this.parser, this.form, this.parseLhs,
  ...
  public static Parse<Term> parse(Input input, ExprParser parser, TermForm<?> form,
    ...
    if (step == 1) {
      if (parseLhs == null) {
        parseLhs = parser.parseBitwiseOrExpr(input, form);
      } else {
    ...
    do {
      ...
      if (step == 4) {
        if (parseRhs == null) {
          parseRhs = parser.parseBitwiseOrExpr(input, form);
        } else {
          parseRhs = parseRhs.consume(input);
        }
        if (parseRhs.isDone()) {
          parseLhs = Parse.done(new AndExpr(Assume.nonNull(parseLhs).getNonNullUnchecked(),
    ...
    return new ParseAndExpr(parser, form, parseLhs, parseRhs, step);
  }
