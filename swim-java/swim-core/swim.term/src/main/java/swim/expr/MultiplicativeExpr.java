...
@Public
@Since("5.0")
public abstract class MultiplicativeExpr extends InfixExpr {

  MultiplicativeExpr(Term lhs, Term rhs) {
    super(lhs, rhs);
  }
  ...
  public static Parse<Object> parse(Input input, TermParser<?> parser,
    ...
    return ParseMultiplicativeExpr.parse(input, parser, options, null, null, null, 1);
  }

}

final class ParseMultiplicativeExpr extends Parse<Object> {
  ...
  ParseMultiplicativeExpr(TermParser<?> parser, TermParserOptions options,
  ...
  public Parse<Object> consume(Input input) {
    return ParseMultiplicativeExpr.parse(input, this.parser, this.options, this.parseLhs,
  ...
  static Parse<Object> parse(Input input, TermParser<?> parser, TermParserOptions options,
    ...
    if (step == 1) {
      if (parseLhs == null) {
        parseLhs = PrefixExpr.parse(input, parser, options);
      } else {
    ...
    do {
      ...
      if (step == 3) {
        ...
        if (parseRhs.isDone()) {
          ...
          if ("*".equals(operator)) {
            parseLhs = Parse.done(new TimesExpr(lhs, rhs));
          } else if ("/".equals(operator)) {
            parseLhs = Parse.done(new DivideExpr(lhs, rhs));
          } else if ("%".equals(operator)) {
            parseLhs = Parse.done(new ModuloExpr(lhs, rhs));
          } else {
    ...
    return new ParseMultiplicativeExpr(parser, options, parseLhs, operator, parseRhs, step);
  }
