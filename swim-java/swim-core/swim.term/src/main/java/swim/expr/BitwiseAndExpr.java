...
@Public
@Since("5.0")
public final class BitwiseAndExpr extends InfixExpr implements WriteSource {

  public BitwiseAndExpr(Term lhs, Term rhs) {
    super(lhs, rhs);
  }
  ...
  public boolean equals(@Nullable Object other) {
    if (this == other) {
      return true;
    } else if (other instanceof BitwiseAndExpr that) {
      return this.lhs.equals(that.lhs) && this.rhs.equals(that.rhs);
    }
    return false;
  }

  private static final int HASH_SEED = Murmur3.seed(BitwiseAndExpr.class);
  ...
  public void writeSource(Appendable output) {
    ...
    notation.beginInvoke("BitwiseAndExpr", "of")
  ...
  public static BitwiseAndExpr of(Term lhs, Term rhs) {
    return new BitwiseAndExpr(lhs, rhs);
  }

  public static Parse<Object> parse(Input input, TermParser<?> parser,
                                    TermParserOptions options) {
    return ParseBitwiseAndExpr.parse(input, parser, options, null, null, 1);
  }

}

final class ParseBitwiseAndExpr extends Parse<Object> {
  ...
  ParseBitwiseAndExpr(TermParser<?> parser, TermParserOptions options,
  ...
  public Parse<Object> consume(Input input) {
    return ParseBitwiseAndExpr.parse(input, this.parser, this.options,
  ...
  static Parse<Object> parse(Input input, TermParser<?> parser, TermParserOptions options,
    ...
    if (step == 1) {
      if (parseLhs == null) {
        parseLhs = ComparisonExpr.parse(input, parser, options);
      } else {
    ...
    do {
      ...
      if (step == 3) {
        if (parseRhs == null) {
          parseRhs = ComparisonExpr.parse(input, parser, options);
        } else {
        ...
        if (parseRhs.isDone()) {
          ...
          parseLhs = Parse.done(new BitwiseAndExpr(lhs, rhs));
          parseRhs = null;
          step = 2;
          continue;
    ...
    return new ParseBitwiseAndExpr(parser, options, parseLhs, parseRhs, step);
  }
