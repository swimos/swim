...
public final class BitwiseXorExpr extends InfixExpr implements ToSource {
  ...
  public boolean equals(@Nullable Object other) {
    if (this == other) {
      return true;
    } else if (other instanceof BitwiseXorExpr that) {
  ...
  public void writeSource(Appendable output) {
    ...
    notation.beginInvoke("BitwiseXorExpr", "of")
  ...
  public static BitwiseXorExpr of(Term lhs, Term rhs) {
    return new BitwiseXorExpr(lhs, rhs);
  }

  public static Parse<Object> parse(Input input, TermParser<?> parser,
                                    TermParserOptions options) {
    return ParseBitwiseXorExpr.parse(input, parser, options, null, null, 1);
  }

}

final class ParseBitwiseXorExpr extends Parse<Object> {
  ...
  ParseBitwiseXorExpr(TermParser<?> parser, TermParserOptions options,
  ...
  public Parse<Object> consume(Input input) {
    return ParseBitwiseXorExpr.parse(input, this.parser, this.options,
  ...
  static Parse<Object> parse(Input input, TermParser<?> parser, TermParserOptions options,
    ...
    if (step == 1) {
      if (parseLhs == null) {
        parseLhs = BitwiseAndExpr.parse(input, parser, options);
      } else {
    ...
    do {
      ...
      if (step == 3) {
        if (parseRhs == null) {
          parseRhs = BitwiseAndExpr.parse(input, parser, options);
        } else {
        ...
        if (parseRhs.isDone()) {
          ...
          parseLhs = Parse.done(new BitwiseXorExpr(lhs, rhs));
          parseRhs = null;
          step = 2;
          continue;
    ...
    return new ParseBitwiseXorExpr(parser, options, parseLhs, parseRhs, step);
  }
