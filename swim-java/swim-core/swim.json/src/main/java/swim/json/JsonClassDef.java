...
import swim.json.decl.JsonAutoDetect;
...
final class JsonClassDef<T> implements WriteMarkup {
  ...
  @Nullable JsonCreatorDef<T> creatorDef;
  UniformMap<String, JsonPropertyDef<T>> propertyDefs;

  JsonClassDef(JsonMetaCodec metaCodec, Class<T> classType) {
    ...
    this.propertyDefs = new UniformMap<String, JsonPropertyDef<T>>();
  }

  @Nullable JsonPropertyDef<T> getPropertyDef(String name) {
    return this.propertyDefs.get(name);
  }

  JsonPropertyDef<T> getOrCreatePropertyDef(String name) {
    JsonPropertyDef<T> propertyDef = this.propertyDefs.get(name);
    if (propertyDef == null) {
      propertyDef = new JsonPropertyDef<T>(this, name);
  ...
  void reflect() throws JsonProviderException {
  ...
  void reflectConstructors(Class<T> classType) throws JsonProviderException {
  ...
  void reflectConstructor(Constructor<T> constructor) throws JsonProviderException {
    final HashTrieMap<Class<?>, Annotation> annotationMap = Json.resolveAnnotations(constructor);
  ...
  void reflectField(Field field) {
    final HashTrieMap<Class<?>, Annotation> annotationMap = Json.resolveAnnotations(field);
  ...
  void reflectInstanceField(Field field, HashTrieMap<Class<?>, Annotation> annotationMap) {
    final JsonPropertyDef<T> propertyDef = this.reflectPropertyField(field, annotationMap);
    if (propertyDef != null) {
      propertyDef.reflectInstanceField(field, annotationMap);
    }
  }

  void reflectMethods(@Nullable Class<?> classType) throws JsonProviderException {
    if (classType == null || classType == Object.class) {
      // Terminate the recursion before reflecting Object.class.
      return;
    }
  ...
  void reflectMethod(Method method) throws JsonProviderException {
    final HashTrieMap<Class<?>, Annotation> annotationMap = Json.resolveAnnotations(method);
  ...
  void reflectStaticMethod(Method method, HashTrieMap<Class<?>, Annotation> annotationMap) throws JsonProviderException {
  ...
  void reflectInstanceMethod(Method method, HashTrieMap<Class<?>, Annotation> annotationMap) throws JsonProviderException {
    final JsonPropertyDef<T> propertyDef = this.reflectPropertyMethod(method, annotationMap);
    if (propertyDef != null) {
  ...
  void reflectAutoDetect(Class<?> classType, HashTrieMap<Class<?>, Annotation> annotationMap) {
    ...
    Visibility autoDetectUpdaters;
    Visibility autoDetectSetters;
    if ((autoDetectAnnotation = annotationMap.get(JsonAutoDetect.class)) != null) {
      autoDetect = ((JsonAutoDetect) autoDetectAnnotation).value();
      autoDetectFields = ((JsonAutoDetect) autoDetectAnnotation).fields();
      autoDetectGetters = ((JsonAutoDetect) autoDetectAnnotation).getters();
      autoDetectUpdaters = ((JsonAutoDetect) autoDetectAnnotation).updaters();
      autoDetectSetters = ((JsonAutoDetect) autoDetectAnnotation).setters();
  ...
  void reflectInitializer(Member member, HashTrieMap<Class<?>, Annotation> annotationMap) {
    ...
    if ((initializerAnnotation = annotationMap.get(JsonInitializer.class)) == null
  ...
  void reflectUnmarshal(Method method, HashTrieMap<Class<?>, Annotation> annotationMap) throws JsonProviderException {
    Annotation unmarshalAnnotation;
    if ((unmarshalAnnotation = annotationMap.get(JsonUnmarshal.class)) == null
        && (unmarshalAnnotation = annotationMap.get(Unmarshal.class)) == null) {
      return;
    }

    if ((method.getModifiers() & Modifier.STATIC) == 0) {
      throw new JsonProviderException("non-static "
                                    + "@" + unmarshalAnnotation.annotationType().getSimpleName()
                                    + " method " + method);
    } else if (method.getParameterCount() != 1) {
      throw new JsonProviderException("@" + unmarshalAnnotation.annotationType().getSimpleName()
                                    + " method " + method + " must have exactly 1 parameter");
    } else if (!this.classType.isAssignableFrom(method.getReturnType())) {
      throw new JsonProviderException("return type of "
                                    + "@" + unmarshalAnnotation.annotationType().getSimpleName()
                                    + " method "
      ...
      throw new JsonProviderException("duplicate "
                                    + "@" + unmarshalAnnotation.annotationType().getSimpleName()
                                    + " method " + method);
    }
  ...
  void reflectMarshal(Method method, HashTrieMap<Class<?>, Annotation> annotationMap) throws JsonProviderException {
    Annotation marshalAnnotation;
    if ((marshalAnnotation = annotationMap.get(JsonMarshal.class)) == null
    ...
    if ((method.getModifiers() & Modifier.STATIC) != 0) {
      if (method.getParameterCount() != 1) {
        throw new JsonProviderException("static "
        ...
        throw new JsonProviderException("parameter type of static "
                                      + "@" + marshalAnnotation.annotationType().getSimpleName()
                                      + " method " + method
      ...
      if (method.getParameterCount() != 0) {
        throw new JsonProviderException("@" + marshalAnnotation.annotationType().getSimpleName()
                                      + " method " + method
  ...
  @Nullable JsonCreatorDef<T> reflectCreator(Executable executable, HashTrieMap<Class<?>, Annotation> annotationMap) throws JsonProviderException {
    Annotation creatorAnnotation;
    if ((creatorAnnotation = annotationMap.get(JsonCreator.class)) == null
    ...
    final JsonCreatorDef<T> creatorDef = new JsonCreatorDef<T>(this, executable);
    creatorDef.reflectParameters();
    this.creatorDef = creatorDef;
    return creatorDef;
  }

  @Nullable JsonPropertyDef<T> reflectPropertyFormat(Method method, HashTrieMap<Class<?>, Annotation> annotationMap) {
    final Annotation propertyFormatAnnotation;
    final String propertyName;
    if ((propertyFormatAnnotation = annotationMap.get(JsonPropertyFormat.class)) != null) {
      propertyName = ((JsonPropertyFormat) propertyFormatAnnotation).value();
    } else {
      return null;
    }
    ...
    final JsonPropertyDef<T> propertyDef = this.getOrCreatePropertyDef(propertyName);
    propertyDef.formatMethod = method;
    return propertyDef;
  }

  @Nullable JsonPropertyDef<T> reflectPropertyField(Field field, HashTrieMap<Class<?>, Annotation> annotationMap) {
  ...
  @Nullable JsonPropertyDef<T> reflectPropertyMethod(Method method, HashTrieMap<Class<?>, Annotation> annotationMap) throws JsonProviderException {
    ...
    if ((getterAnnotation = annotationMap.get(JsonGetter.class)) != null) {
      if (propertyName == null || propertyName.length() == 0) {
        propertyName = ((JsonGetter) getterAnnotation).value();
      }
      getterMethod = method;
    ...
    if ((updaterAnnotation = annotationMap.get(JsonUpdater.class)) != null) {
      if (propertyName == null || propertyName.length() == 0) {
        propertyName = ((JsonUpdater) updaterAnnotation).value();
      }
    ...
    if ((setterAnnotation = annotationMap.get(JsonSetter.class)) != null) {
      if (propertyName == null || propertyName.length() == 0) {
        propertyName = ((JsonSetter) setterAnnotation).value();
      }
      setterMethod = method;
    ...
    if (getterAnnotation != null && updaterAnnotation != null) {
      throw new JsonProviderException("conflicting "
  ...
  void flattenProperties() throws JsonProviderException {
    for (JsonPropertyDef<T> propertyDef : this.propertyDefs.clone().values()) {
      if (propertyDef.ignore || propertyDef.annex || !propertyDef.flatten) {
        continue;
      }
      final UniformMap<String, JsonFieldFormat<?, T>> flattenedFieldFormats =
          propertyDef.toFlattenedFieldFormats();
      if (flattenedFieldFormats == null) {
        continue;
      }
      for (JsonFieldFormat<?, T> flattenedFieldFormat : flattenedFieldFormats.values()) {
        final JsonPropertyDef<T> flattenedPropertyDefs =
  ...
  UniformMap<String, JsonFieldParser<?, T>> toFieldParsers() throws JsonProviderException {
    final UniformMap<String, JsonFieldParser<?, T>> fieldParsers = UniformMap.of();
    for (Map.Entry<String, JsonPropertyDef<T>> entry : this.propertyDefs) {
      final JsonPropertyDef<T> propertyDef = entry.getValue();
      if (propertyDef.ignore || propertyDef.annex) {
        continue;
      }
      final JsonFieldParser<?, T> fieldParser = propertyDef.toFieldParser();
      if (fieldParser == null) {
        continue;
      }
      fieldParsers.put(entry.getKey(), fieldParser);
    }
    for (JsonPropertyDef<T> propertyDef : this.propertyDefs.values()) {
      ...
      for (String alias : propertyDef.aliases) {
        if (fieldParsers.containsKey(alias)) {
          throw new JsonProviderException(Notation.of("non-unique alias ")
                                                  .appendSource(alias)
                                                  .append(" for property ")
  ...
  UniformMap<String, JsonFieldWriter<?, T>> toFieldWriters() throws JsonProviderException {
    final UniformMap<String, JsonFieldWriter<?, T>> fieldWriters = UniformMap.of();
    for (Map.Entry<String, JsonPropertyDef<T>> entry : this.propertyDefs) {
      final JsonPropertyDef<T> propertyDef = entry.getValue();
      if (propertyDef.ignore || propertyDef.annex || propertyDef.flatten) {
        continue;
      }
      final JsonFieldWriter<?, T> fieldWriter = propertyDef.toFieldWriter();
      if (fieldWriter == null) {
        continue;
      }
  ...
  UniformMap<String, JsonFieldFormat<?, T>> toFieldFormats() throws JsonProviderException {
    final UniformMap<String, JsonFieldFormat<?, T>> fieldFormats = UniformMap.of();
    for (Map.Entry<String, JsonPropertyDef<T>> entry : this.propertyDefs) {
      final JsonPropertyDef<T> propertyDef = entry.getValue();
      if (propertyDef.ignore || propertyDef.annex) {
        continue;
      }
      final JsonFieldFormat<?, T> fieldFormat = propertyDef.toFieldFormat();
      if (fieldFormat == null) {
        continue;
      }
  ...
  @Nullable JsonFieldFormat<?, T> toAnnexFieldFormat() throws JsonProviderException {
    JsonFieldFormat<?, T> annexFieldFormat = null;
    for (JsonPropertyDef<T> propertyDef : this.propertyDefs.values()) {
  ...
  @Nullable Supplier<T> toInitializer() throws JsonProviderException {
    if (this.initializerMember instanceof Field) {
      return JsonClassDef.initializer((Field) this.initializerMember);
    } else if (this.initializerMember instanceof Method) {
      return JsonClassDef.initializer((Method) this.initializerMember);
    }
    return null;
  }

  JsonParser<T> toUnmarshallingParser(Method method) throws JsonProviderException {
    // Resolve the JsonFormat of the unmarshal type.
    ...
    final JsonFormat<?> unmarshalFormat;
    try {
      unmarshalFormat = this.metaCodec.getJsonFormat(unmarshalType);
    } catch (JsonProviderException cause) {
      throw new JsonProviderException("no json format for unmarshal type "
    ...
    try {
      ...
      throw new JsonProviderException("inaccessible decode conversion method " + method, cause);
    }
  ...
  JsonWriter<T> toMarshallingWriter(Method method) throws JsonProviderException {
    // Resolve the JsonFormat of the marshal type.
    final Type marshalType = method.getGenericReturnType();
    final Class<?> marshalClass = method.getReturnType();
    final JsonFormat<?> marshalFormat;
    try {
      marshalFormat = this.metaCodec.getJsonFormat(marshalType);
    } catch (JsonProviderException cause) {
      throw new JsonProviderException("no json format for marshal type "
    ...
    try {
      ...
      throw new JsonProviderException("inaccessible encode conversion method " + method, cause);
    }
  ...
  JsonParser<T> toParser() throws JsonProviderException {
    if (this.unmarshalMethod != null) {
      return this.toUnmarshallingParser(this.unmarshalMethod);
    } else if (this.creatorDef != null) {
      return JsonObjectParser.creator(this.typeName, this.creatorDef.executable,
                                      this.toInitializer(), JsonLang.keyFormat(),
      ...
      return JsonObjectParser.mutator(this.typeName, this.constructor,
                                      this.toInitializer(), JsonLang.keyFormat(),
                                      this.toFieldParsers(),
                                      this.toAnnexFieldFormat());
    }
    return JsonParser.unsupported(this.classType);
  }

  JsonWriter<T> toWriter() throws JsonProviderException {
    if (this.marshalMethod != null) {
      return this.toMarshallingWriter(this.marshalMethod);
    }
    return JsonObjectWriter.serializer(this.typeName, this.toFieldWriters(),
                                       this.toAnnexFieldFormat());
  }

  JsonFormat<T> toFormat() throws JsonProviderException {
    final JsonParser<T> parser = this.toParser();
    final JsonWriter<T> writer = this.toWriter();
    if (parser instanceof JsonObjectParser<?, ?, ?>
        && writer instanceof JsonObjectWriter<?, ?>) {
      return JsonObjectFormat.combining(this.typeName, Assume.conforms(parser),
                                        Assume.conforms(writer), this.toFieldFormats());
    }
    return JsonFormat.combining(this.typeName, parser, writer);
  }

  @Override
  public void writeMarkup(Appendable output) {
    final Notation notation = Notation.from(output);
    notation.beginObject("JsonClassDef")
  ...
  static <T> JsonClassDef<T> reflect(JsonMetaCodec metaCodec, Class<T> classType) throws JsonProviderException {
    final JsonClassDef<T> classDef = new JsonClassDef<T>(metaCodec, classType);
    classDef.reflect();
  ...
  static <T> Supplier<T> initializer(@Nullable T value) {
    if (value == null) {
      return Assume.conforms(JsonClassInitializer.NULL);
    }
    return new JsonClassInitializer<T>(value);
  }

  static <T> Supplier<T> initializer(VarHandle fieldHandle) throws JsonProviderException {
    if (fieldHandle.coordinateTypes().size() != 1) {
      throw new JsonProviderException("invalid initializer field handle " + fieldHandle);
    }
    return JsonClassDef.initializer((T) fieldHandle.get());
  }

  static <T> Supplier<T> initializer(Field field) throws JsonProviderException {
    MethodHandles.Lookup lookup = MethodHandles.lookup();
    try {
    ...
    return JsonClassDef.initializer(fieldHandle);
  }

  static <T> Supplier<T> initializer(MethodHandle methodHandle) throws JsonProviderException {
    final MethodType methodType = methodHandle.type();
    if (methodType.parameterCount() != 0) {
      throw new JsonProviderException("invalid initializer method signature " + methodHandle);
    }
  ...
  static <T> Supplier<T> initializer(Method method) throws JsonProviderException {
    MethodHandles.Lookup lookup = MethodHandles.lookup();
    try {
      lookup = MethodHandles.privateLookupIn(method.getDeclaringClass(), lookup);
    } catch (IllegalAccessException | SecurityException cause) {
    ...
    try {
      ...
      throw new JsonProviderException("inaccessible initializer method " + method, cause);
    }

    return JsonClassDef.initializer(methodHandle);
  }
...
final class JsonClassInitializer<T> implements Supplier<T>, WriteSource {
  ...
  JsonClassInitializer(@Nullable T value) {
    this.value = value;
  }
  ...
  public void writeSource(Appendable output) {
    final Notation notation = Notation.from(output);
    notation.beginInvoke("JsonClassDef", "initializer")
