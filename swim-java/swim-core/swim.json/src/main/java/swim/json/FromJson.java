...
/**
 * Annotates a deserialization conversion method that defines JSON
 * deserialization of the enclosing class in terms of a conversion from some
 * other JSON-deserializable type {@code X}, called the <em>interchange type</em>.
 ...
 * deserialization from JSON, such as primitive values, strings, collections,
 * other JSON-deserializable types, etc..
 * <p>
 * The presence of a {@code @FromJson}-annotated method indicates that a
 * {@link JsonForm JsonForm&lt;T&gt;} should be derived that parses instances of
 ...
 * deserialization from JSON string literals. Defining a {@code @FromJson}-annotated
 ...
 * {@code DateTime}-typed properties of JSON-deserializable objects,
 * as {@code <DateTime>} type parameters to generic collections,
 * or by explicit parsing via {@code Json.parse(DateTime.class, input)}.
 *
 * @see JsonConversions
 */
@Public
