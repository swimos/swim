...
public class JsonFormatExprTests {
  ...
  public void evaluateFormatExprs() throws TermException {
    ...
    final FormatExpr expr = FormatExpr.parse("{greeting}, {audience.second}!", Json.metaCodec(), JsonParserOptions.expressions()).getNonNullUnchecked();
  ...
  public static void assertParses(FormatExpr expected, String string) {
    JsonAssertions.assertParses(FormatExpr.parse(Json.metaCodec(), JsonParserOptions.expressions()), expected, string);
  }

  public static void assertWrites(String expected, FormatExpr expr) {
    JsonAssertions.assertWrites(expected, () -> {
      return expr.write(Json.metaCodec(), JsonWriterOptions.readable());
    });
  }
