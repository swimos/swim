...
public final class PlusOperator extends BinaryOperator {
  ...
  public PlusOperator(Item lhs, Item rhs) {
    super(lhs, rhs);
  }
  ...
  public Item evaluate(Interpreter interpreter) {
    ...
    final Item result = lhs.plus(rhs);
  ...
  protected int compareTo(Operator that) {
    if (that instanceof PlusOperator) {
      return this.compareTo((PlusOperator) that);
    }
  ...
  @Override
  public int hashCode() {
    if (PlusOperator.hashSeed == 0) {
      PlusOperator.hashSeed = Murmur3.seed(PlusOperator.class);
    }
    return Murmur3.mash(Murmur3.mix(Murmur3.mix(PlusOperator.hashSeed,
        this.lhs.hashCode()), this.rhs.hashCode()));
  }

  @Override
  public <T> Output<T> debug(Output<T> output) {
    output = output.debug(this.lhs).write('.').write("plus").write('(').debug(this.rhs).write(')');
    return output;
  }
