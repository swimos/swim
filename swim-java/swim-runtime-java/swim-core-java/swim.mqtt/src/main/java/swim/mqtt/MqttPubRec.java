...
public final class MqttPubRec extends MqttPacket<Object> implements Debug {
  ...
  @Override
  public int hashCode() {
    if (MqttPubRec.hashSeed == 0) {
      MqttPubRec.hashSeed = Murmur3.seed(MqttPubRec.class);
    }
    return Murmur3.mash(Murmur3.mix(Murmur3.mix(MqttPubRec.hashSeed, this.packetFlags), this.packetId));
  }

  @Override
  public <T> Output<T> debug(Output<T> output) {
    output = output.write("MqttPubRec").write('.').write("create").write('(').debug(this.packetId).write(')');
  ...
  public static MqttPubRec create(int packetFlags, int packetId) {
    return new MqttPubRec(packetFlags, packetId);
  }

  public static MqttPubRec create(int packetId) {
