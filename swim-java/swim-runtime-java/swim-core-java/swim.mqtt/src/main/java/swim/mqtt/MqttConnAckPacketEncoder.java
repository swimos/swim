...
final class MqttConnAckPacketEncoder extends Encoder<Object, MqttConnAckPacket> {

  final MqttEncoder mqtt;
  final MqttConnAckPacket packet;
  final int length;
  final int remaining;
  final int step;

  MqttConnAckPacketEncoder(MqttEncoder mqtt, MqttConnAckPacket packet,
  ...
  @Override
  public Encoder<Object, MqttConnAckPacket> pull(OutputBuffer<?> output) {
    return MqttConnAckPacketEncoder.encode(output, this.mqtt, this.packet,
                                           this.length, this.remaining, this.step);
  }

  static Encoder<Object, MqttConnAckPacket> encode(OutputBuffer<?> output, MqttEncoder mqtt,
                                                   MqttConnAckPacket packet, int length,
    ...
    return new MqttConnAckPacketEncoder(mqtt, packet, length, remaining, step);
  }

  static Encoder<Object, MqttConnAckPacket> encode(OutputBuffer<?> output, MqttEncoder mqtt,
                                                   MqttConnAckPacket packet) {
    return MqttConnAckPacketEncoder.encode(output, mqtt, packet, 0, 0, 1);
  }
