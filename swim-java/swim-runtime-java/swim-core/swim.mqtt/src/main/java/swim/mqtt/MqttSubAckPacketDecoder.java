...
final class MqttSubAckPacketDecoder extends Decoder<MqttSubAckPacket> {
  ...
  MqttSubAckPacketDecoder(MqttDecoder mqtt, int packetFlags, int packetId,
  ...
  public Decoder<MqttSubAckPacket> feed(InputBuffer input) {
    return MqttSubAckPacketDecoder.decode(input, this.mqtt, this.packetFlags,
                                          this.packetId, this.subscriptions,
  ...
  static Decoder<MqttSubAckPacket> decode(InputBuffer input, MqttDecoder mqtt,
    ...
    if (step == 9 && remaining == 0) {
      return Decoder.done(mqtt.subAckPacket(packetFlags, packetId, subscriptions));
    }
    if (remaining < 0) {
    ...
    return new MqttSubAckPacketDecoder(mqtt, packetFlags, packetId,
                                       subscriptions, remaining, step);
  }

  static Decoder<MqttSubAckPacket> decode(InputBuffer input, MqttDecoder mqtt) {
    return MqttSubAckPacketDecoder.decode(input, mqtt, 0, 0,
                                          FingerTrieSeq.<MqttSubStatus>empty(), 0, 1);
  }
