...
final class MqttPubAckPacketDecoder extends Decoder<MqttPubAckPacket> {
  ...
  MqttPubAckPacketDecoder(MqttDecoder mqtt, int packetFlags, int packetId,
  ...
  public Decoder<MqttPubAckPacket> feed(InputBuffer input) {
    return MqttPubAckPacketDecoder.decode(input, this.mqtt, this.packetFlags,
  ...
  static Decoder<MqttPubAckPacket> decode(InputBuffer input, MqttDecoder mqtt,
    ...
    if (step == 8 && remaining == 0) {
      return Decoder.done(mqtt.pubAckPacket(packetFlags, packetId));
    }
    if (remaining < 0) {
