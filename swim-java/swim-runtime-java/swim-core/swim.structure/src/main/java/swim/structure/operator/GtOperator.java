...
public final class GtOperator extends BinaryOperator {
  ...
  public GtOperator(Item lhs, Item rhs) {
    super(lhs, rhs);
  }
  ...
  protected int compareTo(Operator that) {
    if (that instanceof GtOperator) {
      return this.compareTo((GtOperator) that);
    }
  ...
  @Override
  public int hashCode() {
    if (GtOperator.hashSeed == 0) {
      GtOperator.hashSeed = Murmur3.seed(GtOperator.class);
    }
    return Murmur3.mash(Murmur3.mix(Murmur3.mix(GtOperator.hashSeed,
        this.lhs.hashCode()), this.rhs.hashCode()));
  }

  @Override
  public <T> Output<T> debug(Output<T> output) {
    output = output.debug(this.lhs).write('.').write("gt").write('(').debug(this.rhs).write(')');
    return output;
  }
