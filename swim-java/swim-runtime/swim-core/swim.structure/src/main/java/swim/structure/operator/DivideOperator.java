...
public final class DivideOperator extends BinaryOperator {
  ...
  public DivideOperator(Item lhs, Item rhs) {
    super(lhs, rhs);
  }
  ...
  public Item evaluate(Interpreter interpreter) {
    ...
    final Item result = lhs.divide(rhs);
  ...
  protected int compareTo(Operator that) {
    if (that instanceof DivideOperator) {
      return this.compareTo((DivideOperator) that);
    }
  ...
  @Override
  public int hashCode() {
    if (DivideOperator.hashSeed == 0) {
      DivideOperator.hashSeed = Murmur3.seed(DivideOperator.class);
    }
    return Murmur3.mash(Murmur3.mix(Murmur3.mix(DivideOperator.hashSeed,
        this.lhs.hashCode()), this.rhs.hashCode()));
  }

  @Override
  public <T> Output<T> debug(Output<T> output) {
    output = output.debug(this.lhs).write('.').write("divide").write('(').debug(this.rhs).write(')');
    return output;
  }
