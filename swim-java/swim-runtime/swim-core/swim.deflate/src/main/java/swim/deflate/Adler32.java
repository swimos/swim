...
// Copyright (c) 2015-2023 Swim.inc
//
...
final class Adler32 {
  ...
  static int adler32(int adler, byte[] buffer, int offset, int length) {
    ...
    if (length < 16) {
      ...
      sum2 %= Adler32.BASE; // only added so many BASE's
      return adler | (sum2 << 16);
    }

    // do length NMAX blocks -- requires just one modulo operation
    while (length >= Adler32.NMAX) {
      length -= Adler32.NMAX;
      n = Adler32.NMAX / 16; // NMAX is divisible by 16
      do {
        adler += buffer[offset++]; sum2 += adler;
        adler += buffer[offset++]; sum2 += adler;
        adler += buffer[offset++]; sum2 += adler;
