...
final class MqttConnAckPacketDecoder extends Decoder<MqttConnAckPacket> {
  ...
  MqttConnAckPacketDecoder(MqttDecoder mqtt, int packetFlags, int connectFlags,
  ...
  public Decoder<MqttConnAckPacket> feed(InputBuffer input) {
    return MqttConnAckPacketDecoder.decode(input, this.mqtt, this.packetFlags, this.connectFlags,
  ...
  static Decoder<MqttConnAckPacket> decode(InputBuffer input, MqttDecoder mqtt,
    ...
    if (step == 8 && remaining == 0) {
      return Decoder.done(mqtt.connAckPacket(packetFlags, connectFlags, connectCode));
    }
    if (remaining < 0) {
    ...
    return new MqttConnAckPacketDecoder(mqtt, packetFlags, connectFlags,
                                        connectCode, remaining, step);
  }
