...
final class MqttDisconnectPacketDecoder extends Decoder<MqttDisconnectPacket> {
  ...
  public Decoder<MqttDisconnectPacket> feed(InputBuffer input) {
    return MqttDisconnectPacketDecoder.decode(input, this.mqtt, this.packetFlags,
  ...
  static Decoder<MqttDisconnectPacket> decode(InputBuffer input, MqttDecoder mqtt,
    ...
    if (step == 6 && remaining == 0) {
      return Decoder.done(mqtt.disconnectPacket(packetFlags));
    }
    if (remaining < 0) {
