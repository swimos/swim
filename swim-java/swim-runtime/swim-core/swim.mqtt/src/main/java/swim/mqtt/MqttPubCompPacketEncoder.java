...
final class MqttPubCompPacketEncoder extends Encoder<Object, MqttPubCompPacket> {

  final MqttEncoder mqtt;
  final MqttPubCompPacket packet;
  final int length;
  final int remaining;
  final int step;

  MqttPubCompPacketEncoder(MqttEncoder mqtt, MqttPubCompPacket packet,
  ...
  @Override
  public Encoder<Object, MqttPubCompPacket> pull(OutputBuffer<?> output) {
    return MqttPubCompPacketEncoder.encode(output, this.mqtt, this.packet,
                                           this.length, this.remaining, this.step);
  }

  static Encoder<Object, MqttPubCompPacket> encode(OutputBuffer<?> output, MqttEncoder mqtt,
                                                   MqttPubCompPacket packet, int length,
    ...
    return new MqttPubCompPacketEncoder(mqtt, packet, length, remaining, step);
  }

  static Encoder<Object, MqttPubCompPacket> encode(OutputBuffer<?> output, MqttEncoder mqtt,
                                                   MqttPubCompPacket packet) {
    return MqttPubCompPacketEncoder.encode(output, mqtt, packet, 0, 0, 1);
  }
