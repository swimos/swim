...
final class DoctypeDeclParser<I, V> extends Parser<I> {
  ...
  public Parser<I> feed(Input input) {
    return DoctypeDeclParser.parse(input, this.xml, this.nameParser, this.publicId,
                                   this.systemId, this.markupDeclParser, this.quote, this.step);
  }
    ...
    while (step >= 1 && step <= 9) {
      if (input.isCont()) {
        if (input.head() == "<!DOCTYPE".charAt(step - 1)) {
          ...
          return Parser.error(Diagnostic.expected("<!DOCTYPE".charAt(step - 1), input));
        }
    ...
    while (step >= 14 && step <= 19) {
      if (input.isCont()) {
        if (input.head() == "PUBLIC".charAt(step - 14)) {
          ...
          return Parser.error(Diagnostic.expected("PUBLIC".charAt(step - 14), input));
        }
      } else if (input.isDone()) {
        return Parser.error(Diagnostic.expected("PUBLIC".charAt(step - 14), input));
      }
    ...
    if (step == 22) {
      ...
      if (input.isCont()) {
        if (c == quote) {
          ...
          return Parser.error(Diagnostic.expected(c, input));
        }
      } else if (input.isDone()) {
        return Parser.error(Diagnostic.expected(c, input));
      }
    ...
    while (step >= 23 && step <= 28) {
      if (input.isCont()) {
        if (input.head() == "SYSTEM".charAt(step - 23)) {
          ...
          return Parser.error(Diagnostic.expected("SYSTEM".charAt(step - 23), input));
        }
      } else if (input.isDone()) {
        return Parser.error(Diagnostic.expected("SYSTEM".charAt(step - 23), input));
      }
      ...
      if (input.isCont()) {
        ...
        if (c == '>') {
          ...
          if (publicId == null && systemId == null) {
            return Parser.done(xml.doctype(nameParser.bind()));
          } else if (publicId == null) {
            return Parser.done(xml.doctype(nameParser.bind(), systemId.bind()));
          } else {
            return Parser.done(xml.doctype(nameParser.bind(), publicId.bind(), systemId.bind()));
          }
        } else {
          return Parser.error(Diagnostic.expected('>', input));
        }
  ...
  static <I, V> Parser<I> parse(Input input, XmlParser<I, V> xml) {
    return DoctypeDeclParser.parse(input, xml, null, null, null, null, 0, 1);
  }

  static <I, V> Parser<I> parseRest(Input input, XmlParser<I, V> xml) {
    return DoctypeDeclParser.parse(input, xml, null, null, null, null, 0, 3);
  }
