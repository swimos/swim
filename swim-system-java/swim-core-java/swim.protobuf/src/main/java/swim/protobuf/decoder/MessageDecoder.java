...
final class MessageDecoder<T, M> extends Decoder<T> {
  ...
  final long size;
  final M message;
  final ProtobufFieldType<M, Object> fieldType;
  ...
  MessageDecoder(ProtobufDecoder protobuf, ProtobufMessageType<T, M> type, long size,
                 M message, ProtobufFieldType<M, Object> fieldType, Decoder<?> valueDecoder,
                 long tag, long length, int shift, int step) {
  ...
  MessageDecoder(ProtobufDecoder protobuf, ProtobufMessageType<T, M> type, long size) {
    this(protobuf, type, size, null, null, null, 0L, 0L, 0, size >= 0 ? 1 : 2);
  }

  @Override
  public Decoder<T> feed(InputBuffer input) {
    return decode(input, this.protobuf, this.type, this.size, this.message, this.fieldType,
                  this.valueDecoder, this.tag, this.length, this.shift, this.step);
  }

  @SuppressWarnings("unchecked")
  static <T, M> Decoder<T> decode(InputBuffer input, ProtobufDecoder protobuf,
                                  ProtobufMessageType<T, M> type, long size,
                                  M message, ProtobufFieldType<M, Object> fieldType,
                                  Decoder<?> valueDecoder, long tag, long length,
                                  int shift, int step) {
    if (step == 1) {
      while (input.isCont()) {
      ...
      do {
        if (step == 2) {
          ...
          while (input.isCont()) {
            ...
            if ((b & 0x80) == 0) {
              ...
              fieldType = (ProtobufFieldType<M, Object>) type.getField(fieldNumber);
              if (wireType.isSized() && fieldType.valueType().wireType().isPrimitive()) {
                fieldType = (ProtobufFieldType<M, Object>) fieldType.packedType();
                step = 4;
              } else {
                step = 3;
              }
        ...
        if (step == 3) {
          ...
          if (valueDecoder.isDone()) {
            ...
            message = fieldType.updated(message, valueDecoder.bind());
            fieldType = null;
            valueDecoder = null;
            tag = 0L;
            step = 2;
        ...
        if (step == 5) {
          final int packedInputStart = input.index();
          final int packedInputLimit = input.limit();
          final int packedInputRemaining = packedInputLimit - packedInputStart;
          final boolean packedInputPart = input.isPart();
          if (length < packedInputRemaining) {
            input.limit(packedInputStart + (int) length);
          }
          if (length <= packedInputRemaining) {
            input = input.isPart(false);
          }
          ...
          input = input.limit(packedInputLimit).isPart(packedInputPart);
          length -= input.index() - packedInputStart;
          if (valueDecoder.isDone()) {
            if (message == null) {
              message = type.create();
            }
    ...
    return new MessageDecoder<T, M>(protobuf, type, size, message, fieldType,
                                    valueDecoder, tag, length, shift, step);
  }

  static <T, M> Decoder<T> decode(InputBuffer input, ProtobufDecoder protobuf,
                                  ProtobufMessageType<T, M> type, long size) {
    return decode(input, protobuf, type, size, null, null, null, 0L, 0L, 0, size >= 0 ? 1 : 2);
  }
