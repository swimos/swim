...
public class KeyedWindowStreamletSpec extends ConnectorTest {
  ...
  public void handleUpdatesAndRemovals() {
    ...
    final KeyedWindowStreamlet<String, Pair<Long, Integer>, Integer, Integer> streamlet = new KeyedWindowStreamlet<>(
        schedule, managers, TimestampAssigner.fromData(Pair::getFirst));

    final ArrayList<MapAction<String, Integer>> outputs = createOutputCollector(streamlet);
    ...
    streamlet.notifyChange("a", Deferred.value(in1), Deferred.value(MapView.wrap(inMap)));
    streamlet.notifyChange("b", Deferred.value(in2), Deferred.value(MapView.wrap(inMap.updated("b", in2))));
    streamlet.notifyRemoval("a", Deferred.value(MapView.wrap(HashTrieMap.of("b", in2))));
  ...
  public void replacePaneManagerOnRemoval() {
    ...
    //Push values for two different keys and then remove the first.
    streamlet.notifyChange("a", Deferred.value(in1), Deferred.value(MapView.wrap(HashTrieMap.of("a", in1))));
    streamlet.notifyRemoval("a", Deferred.value(MapView.wrapSimple(HashTrieMap.empty())));
    ...
    streamlet.notifyChange("a", Deferred.value(in2), Deferred.value(MapView.wrap(HashTrieMap.of("a", in2))));
    streamlet.notifyRemoval("a", Deferred.value(MapView.wrapSimple(HashTrieMap.empty())));
  ...
  public void triggerByDataTimestampsForOneKey() {
    ...
    streamlet.notifyChange("a", Deferred.value(in2), Deferred.value(MapView.wrap(inMap.updated("a", in2))));
    Assert.assertTrue(schedule.getScheduled().isEmpty());
    Assert.assertTrue(outputs.isEmpty());
  ...
  private static ArrayList<MapAction<String, Integer>> createOutputCollector(
      final KeyedWindowStreamlet<String, Pair<Long, Integer>, Integer, Integer> streamlet) {
    final ArrayList<MapAction<String, Integer>> outputs = new ArrayList<>();

    streamlet.subscribe(new MapReceptacle<String, Integer>() {
      @Override
