...
public class PartitionStreamletSpec extends ConnectorTest {
  ...
  public void emitsSinglePartitionCorrectly(final boolean withState) {
    ...
    if (withState) {
      ...
      final PartitionStreamlet<Integer, Integer, PartitionSet<Integer>> streamlet =
          new PartitionStreamlet<>(assigner, persister);
      emitsSinglePartitionCorrectly(streamlet);
      Assert.assertEquals(persister.keys(), HashTrieSet.of(2));
      Assert.assertEquals(persister.get(2).intValue(), 7);
    } else {
      final PartitionStreamlet<Integer, Integer, PartitionSet<Integer>> streamlet =
          new PartitionStreamlet<>(assigner, Form.forInteger());

      emitsSinglePartitionCorrectly(streamlet);
    }
  }

  private void emitsSinglePartitionCorrectly(final PartitionStreamlet<Integer, Integer, PartitionSet<Integer>> streamlet) {
    final ArrayList<MapAction<Integer, Integer>> results =
        ConnectorUtilities.pushData(streamlet, 7);
  ...
  public void emitsTwoPartitionsCorrectly(final boolean withState) {
    ...
    if (withState) {
      ...
      emitsTwoPartitionsCorrectly(streamlet);
  ...
  private void emitsTwoPartitionsCorrectly(final PartitionStreamlet<Integer, Integer, PartitionSet<Integer>> streamlet) {
    final ArrayList<MapAction<Integer, Integer>> results =
        ConnectorUtilities.pushData(streamlet, 7);
  ...
  public void updateExistingPartition(final boolean withState) {
    final SimplePartitionAssigner<Integer, Integer, PartitionSet<Integer>> assigner =
        SimplePartitionAssigner.ofMulti(PartitionStreamletSpec::nonTrivialFactors);
    ...
    if (withState) {
      ...
      updateExistingPartition(streamlet);
  ...
  private void updateExistingPartition(final PartitionStreamlet<Integer, Integer, PartitionSet<Integer>> streamlet) {
    final ArrayList<MapAction<Integer, Integer>> results1 = ConnectorUtilities.pushData(streamlet, 8);

    Assert.assertEquals(results1.size(), 2);

    final ArrayList<MapAction<Integer, Integer>> results2 = ConnectorUtilities.pushData(streamlet, 4);
  ...
  public void removeExpiredPartition(final boolean withState) {
    ...
    if (withState) {
      ...
      final PartitionStreamlet<Record, String, PartitionSet<String>> streamlet =
          new PartitionStreamlet<>(assigner, persister);

      removeExpiredPartitions(streamlet);

      Assert.assertTrue(persister.keys().isEmpty());
    } else {
      final PartitionStreamlet<Record, String, PartitionSet<String>> streamlet =
          new PartitionStreamlet<>(assigner, RECORD_FORM);

      removeExpiredPartitions(streamlet);
    }
  }

  private void removeExpiredPartitions(final PartitionStreamlet<Record, String, PartitionSet<String>> streamlet) {
    final ArrayList<MapAction<String, Record>> results = ConnectorUtilities.pushData(streamlet,
