...
public class ReduceFieldsStreamletSpec {

  @Test
  public void addValuesToReduction() {
    final ReduceFieldsStreamlet<Long, Integer, Integer> streamlet = new ReduceFieldsStreamlet<>(
        0, Integer::sum, Integer::sum);

    final ActionAccumulator<Long, Integer> acc = new ActionAccumulator<>();

    final ArrayList<Integer> outputs = ConnectorUtilities.pushData(streamlet,
        acc.update(1000L, 5),
        acc.update(2000L, 7));
  ...
  public void replaceValueInReduction() {
    ...
    final ArrayList<Integer> outputs = ConnectorUtilities.pushData(streamlet, acc.update(1000L, -3));
  ...
  public void removeValueFromReduction() {
    ...
    final ArrayList<Integer> outputs = ConnectorUtilities.pushData(streamlet, acc.remove(1000L));
