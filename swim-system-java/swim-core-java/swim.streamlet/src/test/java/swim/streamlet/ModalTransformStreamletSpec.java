...
public class ModalTransformStreamletSpec {
  ...
  public void transformByMode(final boolean withState) {
    if (withState) {
      ...
      final ModalTransformStreamlet<Integer, Integer, Multiple> streamlet = new ModalTransformStreamlet<>(
          modePersister,
          (Multiple m) -> (Integer n) -> m.getFactor() * n);

      transformByMode(streamlet);
  ...
  private void transformByMode(final ModalTransformStreamlet<Integer, Integer, Multiple> streamlet) {
    final ArrayList<Integer> results = ConnectorUtilities.pushData(streamlet, Either.left(5), Either.right(Multiple.TRIPLE), Either.left(2));
