...
public class ModalFlatMapStreamletSpec extends ConnectorTest {
  ...
  public void emitOutputsOnVariableSchedule(final boolean withState) {
    ...
    if (withState) {
      ...
      final ModalFlatMapStreamlet<Integer, Integer, Parity> streamlet = new ModalFlatMapStreamlet<>(
          modePersister,
          (Parity p) -> (Integer n) -> Arrays.asList(n, n + (p == Parity.ODD ? 1 : 2)),
          schedule, durationPersister);

      emitOutputOnVariableSchedule(schedule, streamlet);
  ...
  private void emitOutputOnVariableSchedule(final FakeSchedule schedule,
                                            final ModalFlatMapStreamlet<Integer, Integer, Parity> streamlet) {
    ...
    streamlet.third().notifyChange(Deferred.value(Parity.EVEN));
    streamlet.first().notifyChange(Deferred.value(5));
