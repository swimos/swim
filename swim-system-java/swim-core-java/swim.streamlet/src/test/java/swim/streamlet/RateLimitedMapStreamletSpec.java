...
public class RateLimitedMapStreamletSpec extends ConnectorTest {
  ...
  public void limitRateOfOutput(final boolean withState) {
    ...
    if (withState) {
      ...
      final RateLimitedMapStreamlet<String, Integer> streamlet = new RateLimitedMapStreamlet<>(
  ...
  private void limitRateOfOutput(final FakeSchedule schedule,
                                 final RateLimitedMapStreamlet<String, Integer> streamlet) {
    ...
    streamlet.first().notifyChange("a", Deferred.value(3), Deferred.value(MapView.wrap(state)));
    state = state.removed("b");
    streamlet.first().notifyRemoval("b", Deferred.value(MapView.wrap(state)));
  ...
  private void changeOutputPeriod(final FakeSchedule schedule,
    ...
    streamlet.second().notifyChange(Deferred.value(Duration.ofSeconds(1)));
