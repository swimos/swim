...
public class SamplingMapStreamletSpec extends ConnectorTest {
  ...
  public void sampleFromOutput(final boolean withState) {
    ...
    if (withState) {
      ...
      final SamplingMapStreamlet<String, Integer> streamlet = new SamplingMapStreamlet<>(
          schedule, periodPersister);

      sampleFromOutput(schedule, streamlet);
  ...
  private void sampleFromOutput(final FakeSchedule schedule, final SamplingMapStreamlet<String, Integer> streamlet) {
    ...
    final MapReceptacle<String, Integer> receptacle = new MapReceptacle<String, Integer>() {
      ...
      public void notifyChange(final String key, final Deferred<Integer> value, final Deferred<MapView<String, Integer>> map) {
        outputs.add(new ConnectorUtilities.Update<>(key, value.get(), map.get()));
      }

      @Override
      public void notifyRemoval(final String key, final Deferred<MapView<String, Integer>> map) {
        outputs.add(new ConnectorUtilities.Remove<>(key, map.get()));
      }
    };
    ...
    streamlet.first().notifyChange("a", Deferred.value(1), Deferred.value(MapView.wrap(state)));
    ...
    streamlet.first().notifyChange("b", Deferred.value(2), Deferred.value(MapView.wrap(state)));
    ...
    streamlet.first().notifyRemoval("a", Deferred.value(MapView.wrap(state)));
  ...
  public void changeOutputPeriod(final boolean withState) {
    ...
    if (withState) {
      ...
      changeOutputPeriod(schedule, streamlet);
  ...
  private void changeOutputPeriod(final FakeSchedule schedule,
                                  final SamplingMapStreamlet<String, Integer> streamlet) {
    final ArrayList<MapAction<String, Integer>> outputs = new ArrayList<>();
    final MapReceptacle<String, Integer> receptacle = new MapReceptacle<String, Integer>() {
      @Override
      public void notifyChange(final String key, final Deferred<Integer> value, final Deferred<MapView<String, Integer>> map) {
    ...
    streamlet.second().notifyChange(Deferred.value(Duration.ofSeconds(10)));
    Assert.assertEquals(schedule.getScheduled().size(), 1);
