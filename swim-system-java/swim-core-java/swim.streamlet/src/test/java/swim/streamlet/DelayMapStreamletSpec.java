...
public class DelayMapStreamletSpec extends ConnectorTest {

  @Test(expectedExceptions = IllegalArgumentException.class)
  public void failsForZeroDelay() {
    new DelayMapStreamlet<>(0);
  }

  @Test(expectedExceptions = IllegalArgumentException.class)
  public void failsForNegativeDelay() {
    new DelayMapStreamlet<>(-1);
  }

  @Test
  public void emitsNothingOnIntialInput() {
    final DelayMapStreamlet<String, Integer> streamlet = new DelayMapStreamlet<>(1);
  ...
  public void emitsPreviousValuesForAKey() {
    final DelayMapStreamlet<String, Integer> streamlet = new DelayMapStreamlet<>(1);
    final ArrayList<MapAction<String, Integer>> results = ConnectorUtilities.pushData(streamlet, update("a", 3), update("a", 5));
  ...
  public void ignoresOtherKeysWithUnderfullBuffers() {
    final DelayMapStreamlet<String, Integer> streamlet = new DelayMapStreamlet<>(1);
    final ArrayList<MapAction<String, Integer>> results = ConnectorUtilities.pushData(streamlet,
  ...
  public void restoreFromState() {
    ...
    final DelayMapStreamlet<String, Integer> streamlet = new DelayMapStreamlet<>(buffers::get, keys, 1);

    final ArrayList<MapAction<String, Integer>> results = ConnectorUtilities.pushData(streamlet, update("b", 7));
  ...
  public void storeKeysAndBuffersInState() {
    ...
    final DelayMapStreamlet<String, Integer> streamlet = new DelayMapStreamlet<>(fac, keys, 1);

    ConnectorUtilities.pushData(streamlet, update("a", 1), update("b", 2), update("b", 3), update("b", 4));
  ...
  public void removalResetsBuffers() {
    ...
    ConnectorUtilities.pushData(
        streamlet, update("a", 3), update("a", 5));

    final ArrayList<MapAction<String, Integer>> results = ConnectorUtilities.pushData(
        streamlet, remove("a"), update("a", 7));
