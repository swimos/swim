...
public class FlatMapEntriesStreamletSpec extends ConnectorTest {

  @Test
  public void transformEntries() {
    final FlatMapEntriesStreamlet<Integer, Integer, Integer, Integer> streamlet = new FlatMapEntriesStreamlet<>(
        FlatMapEntriesStreamletSpec::nonTrivialFactors, k -> Collections.emptySet());

    final ArrayList<MapAction<Integer, Integer>> results =
        ConnectorUtilities.pushData(streamlet, update(1, 6));
  ...
  public void removeEntries() {
    final FlatMapEntriesStreamlet<Integer, Integer, Integer, Integer> streamlet = new FlatMapEntriesStreamlet<>(
        FlatMapEntriesStreamletSpec::id, FlatMapEntriesStreamletSpec::lessThan);

    final ActionAccumulator<Integer, Integer> acc =
        new ActionAccumulator<>();

    ConnectorUtilities.pushData(streamlet, acc.update(1, 6), acc.update(3, 4), acc.update(7, 7));

    final ArrayList<MapAction<Integer, Integer>> results =
        ConnectorUtilities.pushData(streamlet, acc.remove(3));
