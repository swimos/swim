...
public class PeriodicStreamletSpec extends ConnectorTest {
  ...
  public void limitRateOfOutput() {
    ...
    final PeriodicStreamlet<String> streamlet = new PeriodicStreamlet<>(schedule,
    ...
    streamlet.notifyChange(Deferred.value("The"));

    Assert.assertEquals(schedule.getScheduled().size(), 1);
    Assert.assertTrue(schedule.getScheduled().containsKey(5000L));
    streamlet.notifyChange(Deferred.value("cat"));
    streamlet.notifyChange(Deferred.value("sat"));
    ...
    streamlet.notifyChange(Deferred.value("on"));
    streamlet.notifyChange(Deferred.value("the"));
    streamlet.notifyChange(Deferred.value("mat."));
