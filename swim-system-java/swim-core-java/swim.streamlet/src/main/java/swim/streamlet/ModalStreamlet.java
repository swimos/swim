...
/**
 * Base class for streamlets that have an auxiliary control stream to modify their behaviour.
 *
 ...
 */
public abstract class ModalStreamlet<In, Out, Mode, State> extends AbstractJunction2<In, Mode, Out> implements Junction2<In, Mode, Out> {

  /**
   * Change current mode of the streamlet.
   *
   ...
   */
  protected ModalStreamlet(final State init) {
   ...
   */
  protected ModalStreamlet(final ValuePersister<Mode> modePersister, final Deferred<State> init) {
    state = init;
