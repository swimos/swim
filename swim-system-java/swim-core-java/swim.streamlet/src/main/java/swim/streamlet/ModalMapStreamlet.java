...
/**
 * Base class for map streamlets that have an auxiliary control stream to modify their behaviour.
 *
 ...
 */
public abstract class ModalMapStreamlet<KeyIn, KeyOut, ValIn, ValOut, Mode, State> extends AbstractMapJunction2<KeyIn, KeyOut, ValIn, ValOut, Mode> {
   ...
   */
  protected ModalMapStreamlet(final State init) {
    state = Deferred.value(init);
    this.modePersister = null;
  }

  protected ModalMapStreamlet(final ValuePersister<Mode> modePersister, final State init) {
