...
final class MqttPubCompDecoder extends Decoder<MqttPubComp> {
  ...
  MqttPubCompDecoder(MqttDecoder mqtt, int packetFlags, int packetId,
  ...
  public Decoder<MqttPubComp> feed(InputBuffer input) {
    return MqttPubCompDecoder.decode(input, this.mqtt, this.packetFlags,
  ...
  static Decoder<MqttPubComp> decode(InputBuffer input, MqttDecoder mqtt,
    ...
    if (step == 8 && remaining == 0) {
      return Decoder.done(mqtt.pubComp(packetFlags, packetId));
    }
    if (remaining < 0) {
  ...
  static Decoder<MqttPubComp> decode(InputBuffer input, MqttDecoder mqtt) {
    return MqttPubCompDecoder.decode(input, mqtt, 0, 0, 0, 1);
  }
