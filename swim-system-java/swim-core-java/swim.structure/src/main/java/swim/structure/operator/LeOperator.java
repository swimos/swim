...
public class LeOperator extends BinaryOperator {
  ...
  public LeOperator(Item lhs, Item rhs) {
    super(lhs, rhs);
  }
  ...
  protected int compareTo(Operator that) {
    if (that instanceof LeOperator) {
      return this.compareTo((LeOperator) that);
    }
  ...
  @Override
  public int hashCode() {
    if (LeOperator.hashSeed == 0) {
      LeOperator.hashSeed = Murmur3.seed(LeOperator.class);
    }
    return Murmur3.mash(Murmur3.mix(Murmur3.mix(LeOperator.hashSeed,
        this.lhs.hashCode()), this.rhs.hashCode()));
  }

  @Override
  public <T> Output<T> debug(Output<T> output) {
    output = output.debug(this.lhs).write('.').write("le").write('(').debug(this.rhs).write(')');
    return output;
  }
