...
public final class ExtractHostDirective implements WebRoute {

  final Function<UriHost, WebRoute> eval;

  public ExtractHostDirective(Function<UriHost, WebRoute> eval) {
    this.eval = eval;
  }
  ...
  public WebResponse routeRequest(WebRequest request) {
    ...
    return this.eval.apply(UriHost.parse(header.value())).routeRequest(request);
  }
}
