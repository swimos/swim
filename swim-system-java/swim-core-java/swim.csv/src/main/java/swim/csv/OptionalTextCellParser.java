...
final class OptionalTextCellParser extends Parser<Item> {
  final Value key;
  final Output<Text> output;

  OptionalTextCellParser(Value key, Output<Text> output) {
    this.key = key;
    this.output = output;
  }

  OptionalTextCellParser(Value key) {
    this(key, null);
  }
  ...
  static Parser<Item> parse(Input input, Value key, Output<Text> output) {
    ...
    return new OptionalTextCellParser(key, output);
  }
