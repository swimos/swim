...
final class OrOperatorParser<I, V> extends Parser<V> {
  ...
  OrOperatorParser(ReconParser<I, V> recon, Builder<I, V> builder, Parser<V> lhsParser,
    ...
    do {
      ...
      if (step == 3) {
        ...
        if (rhsParser.isDone()) {
          ...
          lhsParser = done(recon.or(lhs, rhs));
          rhsParser = null;
          step = 2;
          continue;
  ...
  static <I, V> Parser<V> parse(Input input, ReconParser<I, V> recon, Builder<I, V> builder) {
    return OrOperatorParser.parse(input, recon, builder, null, null, 1);
  }
