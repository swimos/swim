...
public class RemoteWarpHost extends AbstractTierBinding implements HostBinding, WarpSocket {
  ...
  public RemoteWarpHost(Uri requestUri, Uri baseUri) {
  ...
  public RemoteWarpHost(Uri baseUri) {
  ...
  PushRequest createPushRequest(Envelope envelope, float prio) {
    return new RemoteWarpHostPushDown(this, envelope, prio);
  }

  PullRequest<Envelope> createPullEnvelope(Envelope envelope, float prio, PushRequest delegate) {
    return new RemoteWarpHostPushUp(this, envelope, prio, delegate);
  }
  ...
  static final AtomicIntegerFieldUpdater<RemoteWarpHost> FLAGS =
      AtomicIntegerFieldUpdater.newUpdater(RemoteWarpHost.class, "flags");

  static final AtomicReferenceFieldUpdater<RemoteWarpHost, Identity> REMOTE_IDENTITY =
      AtomicReferenceFieldUpdater.newUpdater(RemoteWarpHost.class, Identity.class, "remoteIdentity");

  @SuppressWarnings("unchecked")
  static final AtomicReferenceFieldUpdater<RemoteWarpHost, HashTrieMap<Uri, HashTrieMap<Uri, RemoteWarpDownlink>>> DOWNLINKS =
      AtomicReferenceFieldUpdater.newUpdater(RemoteWarpHost.class, (Class<HashTrieMap<Uri, HashTrieMap<Uri, RemoteWarpDownlink>>>) (Class<?>) HashTrieMap.class, "downlinks");

  @SuppressWarnings("unchecked")
  static final AtomicReferenceFieldUpdater<RemoteWarpHost, HashTrieMap<Uri, HashTrieMap<Uri, HashTrieSet<RemoteWarpUplink>>>> UPLINKS =
      AtomicReferenceFieldUpdater.newUpdater(RemoteWarpHost.class, (Class<HashTrieMap<Uri, HashTrieMap<Uri, HashTrieSet<RemoteWarpUplink>>>>) (Class<?>) HashTrieMap.class, "uplinks");
...
final class RemoteWarpHostPushDown implements PushRequest {
  final RemoteWarpHost host;
  final Envelope envelope;
  final float prio;

  RemoteWarpHostPushDown(RemoteWarpHost host, Envelope envelope, float prio) {
    this.host = host;
...
final class RemoteWarpHostPushUp implements PullRequest<Envelope> {
  final RemoteWarpHost host;
  final Envelope envelope;
  final float prio;
  final PushRequest delegate;

  RemoteWarpHostPushUp(RemoteWarpHost host, Envelope envelope, float prio, PushRequest delegate) {
    this.host = host;
    this.envelope = envelope;
    this.prio = prio;
